...
import { bs, Ch, Ea, Jf, $l, gp, normalizeFolder, trashDocument } from '../../data/files.js';
...
export function FilesV2({ storage, onBack = () => undefined, onOpenTrash = () => undefined, onTabChange = () => undefined }: Props) {
  ...
  return <main className="v2-root v2-files" aria-labelledby="files-v2-title"><div className="v2-screen v2-files__screen">
    ...
    {visibleDocuments.length > 0 && <section className="v2-files__section"><div className="v2-files__section-head"><div><span className="v2-eyebrow">MATERIAIS</span><h2>{filter === 'recentes' ? 'Recentes' : 'Arquivos'}</h2></div><span className="v2-files__count">{visibleDocuments.length}</span></div><div className="v2-files__document-list">{visibleDocuments.map((document) => <div className="v2-files__document-row" key={document.id}><span className="v2-files__document-icon"><FileText size={21} /></span><span className="v2-files__document-copy"><strong>{document.nome}</strong><small>{fileKind(document)} · {fileSize(document.tamanho)}</small></span><button className="v2-files__star v2-pressable" type="button" aria-label={document.favorito ? `Remover ${document.nome} dos favoritos` : `Favoritar ${document.nome}`} onClick={() => void toggleFavorite(document)}><Star size={18} fill={document.favorito ? '#ee9e24' : 'none'} /></button><button className="v2-files__delete v2-pressable" type="button" aria-label={`Mover ${document.nome} para a lixeira`} onClick={() => removeDocument(document)}><Trash2 size={17} /></button></div>)}</div></section>}

...
type Props = {
  ...
  onRestorePlan?: (plan: LessonPlan) => void;
  onCreatePlan?: () => void;
...
export function PlanningOverviewV2({ className = 'Sua turma', plans, activities = [], activityError = '', loading = false, error = '', offline = false, onBack = () => undefined, onRetry = () => undefined, onOpenPlan = () => undefined, onRestorePlan = () => undefined, onCreatePlan = () => undefined, onCreateActivity = () => undefined, onOpenActivity = () => undefined, onViewChange = () => undefined, onTabChange = () => undefined }: Props) {
  ...
  const activeActivities = React.useMemo(() => activities.filter((activity) => activity.status !== 'arquivada'), [activities]);
  const archivedPlans = React.useMemo(() => plans.filter((plan) => plan.arquivadoEm).sort((a, b) => (b.arquivadoEm || '').localeCompare(a.arquivadoEm || '')), [plans]);
  ...
  return <main className="v2-root v2-planning-overview" aria-labelledby="planning-overview-v2-title">
    <div className="v2-screen v2-planning-overview__screen">
      ...
      {activityError && <div className="v2-planning-overview__notice is-error" role="alert"><span>{activityError}</span><button type="button" onClick={onRetry}>Tentar novamente</button></div>}
      ...
      {activeActivities.length > 0 && <section className="v2-planning-overview__activities" aria-labelledby="planning-overview-activities-title"><div className="v2-planning-overview__section-heading"><div><span className="v2-eyebrow">PARA LEVAR À TURMA</span><h2 id="planning-overview-activities-title">Atividades preparadas</h2></div><span className="v2-planning-overview__archived-count">{activeActivities.length}</span></div>{activeActivities.slice(0, 4).map((activity) => <button className="v2-planning-overview__activity-row v2-pressable" type="button" key={activity.id} onClick={() => onOpenActivity(activity)}><span className="v2-planning-overview__activity-mark"><BookOpenCheck size={19} /></span><span className="v2-planning-overview__plan-copy"><strong>{activity.titulo}</strong><small>{activity.disciplina || 'Atividade pedagógica'} · {activity.status === 'pronta' ? 'Pronta' : 'Rascunho'}</small></span><ChevronRight size={21} /></button>)}</section>}
      {archivedPlans.length > 0 && <section className="v2-planning-overview__archived" aria-labelledby="planning-overview-archived-title"><div className="v2-planning-overview__section-heading"><div><span className="v2-eyebrow">ARQUIVO</span><h2 id="planning-overview-archived-title">Planos arquivados</h2></div><span className="v2-planning-overview__archived-count">{archivedPlans.length}</span></div>{archivedPlans.slice(0, 3).map((plan) => <article className="v2-planning-overview__archived-row" key={plan.id}><button className="v2-planning-overview__archived-open v2-pressable" type="button" onClick={() => onOpenPlan(plan)}><span className="v2-planning-overview__plan-mark"><BookOpenCheck size={20} /></span><span className="v2-planning-overview__plan-copy"><strong>{plan.tituloTema || 'Plano sem título'}</strong><small>{dateLabel(plan.dataKey)} · {statusLabel(plan)}</small></span></button><button className="v2-planning-overview__restore v2-pressable" type="button" onClick={() => onRestorePlan(plan)}>Restaurar</button></article>)}</section>}
      <div className="v2-planning-overview__quick-actions"><button className="v2-planning-overview__quick-add v2-pressable" type="button" onClick={onCreatePlan}><Plus size={19} /> Preparar outro plano</button><button className="v2-planning-overview__quick-add is-activity v2-pressable" type="button" onClick={onCreateActivity}><BookOpenCheck size={18} /> Criar atividade</button></div>

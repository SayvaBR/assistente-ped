import { ArrowLeft, BookOpenCheck, CalendarDays, ChevronLeft, ChevronRight, FileText, House, LayoutGrid, Lightbulb, LoaderCircle, Plus, Users } from 'lucide-react';
...
export function PlanningDayV2({ className = 'Sua turma', dateKey, plans, activities = [], loading = false, error = '', offline = false, onBack = () => undefined, onRetry = () => undefined, onDateChange = () => undefined, onViewChange = () => undefined, onOpenPlan = () => undefined, onOpenActivity = () => undefined, onCreatePlan = () => undefined, onCreateActivity = () => undefined, onTabChange = () => undefined, activeTab = 'planejamento' }: PlanningDayV2Props) {
  const activePlans = plans.filter((plan) => !plan.arquivadoEm);
  const moments = activePlans.flatMap((plan) => (plan.momentos || []).map((moment) => ({ plan, moment, time: formatTime(moment.horario || plan.horaInicio) }))).filter((item) => item.time).sort((a, b) => a.time.localeCompare(b.time));
  const rows = moments.length ? moments : activePlans.filter((plan) => plan.horaInicio).map((plan) => ({ plan, moment: { titulo: plan.tituloTema || 'Plano de aula', descricao: plan.objetivoGeral || '', horario: plan.horaInicio }, time: formatTime(plan.horaInicio) }));
  return <main className="v2-root v2-planning-day" aria-labelledby="planning-day-v2-title"><div className="v2-screen v2-planning-day__screen">
    <header className="v2-planning-day__header"><button className="v2-planning-day__back v2-pressable" type="button" onClick={onBack} aria-label="Voltar"><ArrowLeft size={25} strokeWidth={2.5} /></button><div><span className="v2-eyebrow">{className}</span><h1 id="planning-day-v2-title">Planejamento diário</h1><p>{formatDate(dateKey)}</p></div><button className="v2-planning-day__calendar v2-pressable" type="button" onClick={() => onTabChange('planejamento')} aria-label="Abrir planejamento"><CalendarDays size={24} /></button></header>
    {offline && <div className="v2-planning-day__notice" role="status">Você está offline. Seus planos continuam disponíveis neste dispositivo.</div>}{error && <div className="v2-planning-day__notice is-error" role="alert"><span>{error}</span><button type="button" onClick={onRetry}>Tentar novamente</button></div>}
    ...
    {rows.length > 0 && <button className="v2-planning-day__add v2-pressable" type="button" onClick={onCreatePlan}><Plus size={19} /> Adicionar momento</button>}
    {activities.length > 0 && <section className="v2-planning-day__activities" aria-labelledby="planning-day-activities-title"><div className="v2-planning-day__activities-heading"><div><span className="v2-eyebrow">PREPARO</span><h2 id="planning-day-activities-title">Atividades para levar</h2></div><span>{activities.length}</span></div>{activities.map((activity) => <button className="v2-planning-day__activity v2-pressable" type="button" key={activity.id} onClick={() => onOpenActivity(activity)}><span className="v2-planning-day__plan-icon"><BookOpenCheck size={20} /></span><span><strong>{activity.titulo}</strong><small>{activity.disciplina || 'Atividade pedagógica'} · {activity.status === 'pronta' ? 'Pronta' : 'Rascunho'}</small></span><ChevronRight size={21} /></button>)}</section>}
    <section className="v2-planning-day__next-step" aria-labelledby="planning-day-next-step-title"><div><span className="v2-eyebrow">PRÓXIMO PASSO</span><h2 id="planning-day-next-step-title">Deixe a próxima aula pronta</h2><p>Transforme um objetivo em uma atividade para levar com você.</p><button className="v2-planning-day__next-action v2-pressable" type="button" onClick={onCreateActivity}>Criar atividade <ChevronRight size={18} /></button></div><span className="v2-planning-day__next-mark" aria-hidden="true"><Lightbulb size={28} /></span></section>

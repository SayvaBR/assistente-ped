...
export function PlanningDayV2({ className = 'Sua turma', dateKey, plans, activities = [], loading = false, error = '', offline = false, onBack = () => undefined, onRetry = () => undefined, onDateChange = () => undefined, onViewChange = () => undefined, onOpenPlan = () => undefined, onOpenActivity = () => undefined, onCreatePlan = () => undefined, onTabChange = () => undefined, activeTab = 'planejamento' }: PlanningDayV2Props) {
  const moments = plans.flatMap((plan) => (plan.momentos || []).map((moment) => ({ plan, moment, time: formatTime(moment.horario || plan.horaInicio) }))).filter((item) => item.time).sort((a, b) => a.time.localeCompare(b.time));
  const rows = moments.length ? moments : plans.filter((plan) => plan.horaInicio).map((plan) => ({ plan, moment: { titulo: plan.tituloTema || 'Plano de aula', descricao: plan.objetivoGeral || '', horario: plan.horaInicio }, time: formatTime(plan.horaInicio) }));

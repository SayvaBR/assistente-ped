...
export function ActivityV2({ activity, className = 'Sua turma', offline = false, isEditing = false, onBack, onSalvar, onSaved = () => undefined, onTabChange = () => undefined }: Props) {
  ...
  const save = async () => { setSaving(true); setError(''); setMessage(''); try { await onSalvar({ ...draft, status: 'pronta' }); setMessage('Atividade salva no dispositivo.'); window.setTimeout(onSaved, 650); } catch (cause) { setError(cause instanceof Error ? cause.message : 'Não foi possível salvar a atividade.'); } finally { setSaving(false); } };
